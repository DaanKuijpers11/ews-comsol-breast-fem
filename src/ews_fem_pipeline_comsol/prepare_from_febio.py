...
def _resolve_base_case_toml(comsol_case_toml: Path, settings: Settings) -> Path:
    if not settings.source.base_case_toml:
        raise ValueError(
            ...
            "Point it to a FEBio-style case TOML."
        )
...
def _export_lobules(case_name: str, output_dir: Path, febio_settings) -> Path:
    lobule_path = output_dir / f"{case_name}_lobules.json"
    hetero = febio_settings.material.glandular.hetero
...
def prepare_case_from_febio(
    *,
    ...
    """
    Reuse FEBio prepare_simulation to generate COMSOL-ready input artefacts.
    """
    ...
    if settings.source.export_lobules_json:
        lobules_json = _export_lobules(case_name, output_dir, febio_settings)
        artefacts["lobules_json"] = str(lobules_json.resolve())

    build_plan = {
        "case_name": case_name,
        "geometry": febio_settings.model.geometry.model_dump(),
        "mesh": febio_settings.model.mesh.model_dump(),
        "material": {
            "skin": febio_settings.material.skin.model_dump(),
            "adipose": febio_settings.material.adipose.model_dump(),
            "glandular": febio_settings.material.glandular.model_dump(),
        },
        "lobules": [lob.model_dump() for lob in febio_settings.material.glandular.hetero.build_lobules()],
    }
