...
The model was developed in stages so that geometry, tissue distribution, support assumptions, and lesion effects could be separated. Table~\ref{tab:stage_overview} summarizes the current interpretation of each stage.

\begin{table}[H]
\centering
\small
\caption{Current interpretation of the COMSOL model-development stages.}
\label{tab:stage_overview}
\begin{tabular}{p{0.12\linewidth} p{0.31\linewidth} p{0.42\linewidth}}
\toprule
Stage & Main purpose & Report interpretation \\
\midrule
Stage 1 & Motion sanity baseline & Validates the 0.25g fixed-support acceleration pulse; not a direct anatomical comparator because volume and geometry differ. \\
Stage 2 & Selected chestwall geometry & First fair anatomical baseline using volume-preserving transverse chestwall curvature with xoffset055 and auto-alignment. \\
Stage 3 & Realistic glandular reference & Adds chestwall-aware realistic lobule spread; anatomically useful, but still described carefully as a diagnostic/reference route. \\
Stage 4 & Asymmetry control and sensitivity & The current Tier 1 reference is intentionally no-asymmetry; active asymmetry cases remain sensitivity studies. \\
Stage 5 & Cooper control and support sensitivity & No-Cooper control is valid; a stable Cooper-effect sensitivity has not yet been demonstrated. \\
Stage 6 & Tumor/lesion sensitivity & Build-only and screening stage using analytic tumor-mask material overlays; not yet a segmented tumor reconstruction. \\
\bottomrule
\end{tabular}
\end{table}
